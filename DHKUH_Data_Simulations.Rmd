...
simulate_card_file <- function(card_num, samples_on_card, out_dir = "./simulated_data/simulated_cards") {
  ...
  df <- expand_grid(
    ...
    mutate(
      is_ARG = grepl("bla|tet|sul|qnr|mecA|ARG", Target, ignore.case = TRUE),
      
      # Assign realistic detection probabilities
      detect_prob = case_when(
        sample_type == "NTC" ~ 0.00,
        sample_type == "effluent" & is_ARG ~ 0.70,
        sample_type == "effluent" ~ 0.55,
        sample_type == "compost" & is_ARG ~ 0.40,
        sample_type == "compost" ~ 0.45,
        sample_type == "produce" & is_ARG ~ 0.15,
