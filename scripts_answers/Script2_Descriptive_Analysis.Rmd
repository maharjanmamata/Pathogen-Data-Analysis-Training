...
# Install packages
#install.packages(c("here","tidyverse","janitor","patchwork","pheatmap"))
...
```
#### Plot heatmap of ARG detection for each samples and all targets
```{r}
# Plot as a heatmap
tac_clean %>% 
  filter(Category == "ARG") %>% 
  ggplot(aes(x = household_id, y = Target, fill = as.factor(detect))) +
  geom_tile(color = "white") +
